...
public class Map4 {
    ...
    public Map4(int xMax, int yMax, int startX, int startY, int endX, int endY) {
        this.xMax = xMax;
        this.yMax = yMax;
    ...
    public Map4(boolean[][] map, int startX, int startY, int endX, int endY) {
    ...
    private boolean getNeighbors(Point current) {
        ...
        for (int i = x - 1; i <= x + 1; i++) {
            for (int j = y - 1; j <= y + 1; j++) {
                ...
                if ((i == x && j == y) ||((Math.abs(i-x) + Math.abs(j)-y))==2 || 
